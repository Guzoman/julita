    ...
    for i, sql in enumerate(create_tables_sql, 1):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                print(f"  ✓ SQL {i} ejecutado correctamente")
                successful += 1
            else:
                print(f"  ✗ SQL {i} falló: {response.status_code}")
                if response.text:
    ...
    for table in tables_to_test:
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                print(f"  ✗ Tabla {table} - INSERT falló: {response.status_code}")
                if response.text:
                    print(f"    Error: {response.text[:100]}...")
                
        except Exception as e:
            print(f"  ✗ Error probando {table}: {e}")
...
def insert_sample_data():
    ...
    for item in sample_data:
        try:
            ...
            if response.status_code in [200, 201]:
                print(f"  ✓ Insertado en {item['table']}: {item['data']}")
                inserted += 1
            else:
                print(f"  ✗ Error insertando en {item['table']}: {response.status_code}")
